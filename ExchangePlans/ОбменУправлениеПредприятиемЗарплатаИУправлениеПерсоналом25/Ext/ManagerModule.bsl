...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат НСтр("ru = 'Настройки обмена ERP-ЗУП'");
...
Функция ОписаниеЗначенийПоУмолчанию(ЗначенияПоУмолчаниюНаУзле) Экспорт
	...
	ТекстОписания = НСтр("ru = 'Полученные новые документы будут проводиться, если не требуют ручного дозаполнения;'");
...
Процедура ИнициализироватьРежимыВыгрузки() Экспорт
	
	Запрос = Новый Запрос("
		|ВЫБРАТЬ
		|	ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.Ссылка,
		|	ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.ПравилаОтправкиДокументов,
		|	ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.ПравилаОтправкиСправочников,
		|	ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.РежимВыгрузкиСправочников,
		|	ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.ПометкаУдаления
		|ИЗ
		|	ПланОбмена.ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25 КАК ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25
		|ГДЕ
		|	(ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.ПравилаОтправкиДокументов = &ПустаяСтрока
		|			ИЛИ ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.ПравилаОтправкиСправочников = &ПустаяСтрока
		|			ИЛИ ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.РежимВыгрузкиСправочников = ЗНАЧЕНИЕ(Перечисление.РежимыВыгрузкиОбъектовОбмена.ПустаяСсылка))
		|	И ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.ПометкаУдаления = ЛОЖЬ");
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = '1С:ERP Управление предприятияем, ред. 2.0'");
...
Функция КраткаяИнформацияПоОбмену() Экспорт
	...
	ПоясняющийТекст = НСтр("ru = 'Позволяет синхронизировать данные с конфигурацией ""1С:Зарплата и управление персоналом""'");
...
Функция ПодробнаяИнформацияПоОбмену() Экспорт
	...
	Возврат "ПланОбмена.ОбменУправлениеПредприятиемЗарплатаИУправлениеПерсоналом25.Форма.ПодробнаяИнформация";
