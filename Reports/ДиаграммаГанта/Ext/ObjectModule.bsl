...
Процедура ВывестиИнтервалы(Выборка, СтруктураПараметров, Точка)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Серия") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			НачИнтервала = НачалоДня(Макс(Выборка.НачалоИнтервала, ДатаНач));
			
			КонИнтервала = КонецДня(?(Выборка.КонецИнтервала = NULL, ДатаКон, Мин(Выборка.КонецИнтервала, ДатаКон)));
...
Функция  Печать(Регистратор = "") Экспорт
	...
	Возврат РаботаСДиалогами.НапечататьДокумент(ДокументРезультат, ,, "Диаграмма: " + ВидОтчета);
