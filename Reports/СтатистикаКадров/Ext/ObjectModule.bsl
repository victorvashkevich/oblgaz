...
Функция СформироватьЗаголовокОтчета(Отчет) Экспорт
	...
	Для Сч = 1 По 3 Цикл
		...
		Если Префикс = "Верх" Тогда
			Если ОбщегоНазначения.ЕстьРеквизитОтчета("ВидОтчета",Отчет.Метаданные()) Тогда
...
Процедура ОбработатьПоляБыстрогоОтбораНаФорме(МассивТипов,ЭлементыФормы,СтруктураСвязиЭлементовСДанными,ТаблицаВсехФильтров) Экспорт
	...
	Для каждого ИмяТипа Из МассивТипов Цикл
		Если ИмяТипа = "Статус" Тогда
			СтрокаОтбора = ТаблицаВсехФильтров.Найти(Новый ОписаниеТипов("ПеречислениеСсылка.СостоянияКандидатаНаРаботу"),"ОписаниеТипов");
		Иначе
...
Функция ПолучитьТекстДляСтрокиОтбора(ОписаниеПоля,пВидСравнения,ТекстСписокПараметров,ЗначениеС,ЗначениеПо) Экспорт
	...
	ТекстПоляЗапроса = "";
		Если ОписаниеПоля = "РаботникиОрганизаций.ПодразделениеОрганизации"  
  		...
  		Тогда
		ОписаниеПоля =	 		 		
		" ВЫБОР КОГДА РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) И РаботникиОрганизаций.ПериодЗавершения <= &ДатаАктуальности Тогда " + ОписаниеПоля +"Завершения Иначе " + ОписаниеПоля + " КОНЕЦ ";	
		...
		Тогда
		ОписаниеПоля =	 		 
		" ВЫБОР КОГДА РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) И РаботникиОрганизаций.ПериодЗавершения <= &ДатаАктуальности Тогда " + ОписаниеПоля +" Иначе " + СтрЗаменить(ОписаниеПоля,"Завершения","") + " КОНЕЦ ";	
	КонецЕсли;
...
Функция СтатистикаСпискиПолучитьТекстИсточникаПоИмени(Отчет, Источник, КопияИнтервалыГруппировокОтчета, ТаблицаИсточниковСвойствКатегорий) Экспорт
	...
	Для Каждого Поле из Источник.ТаблицаПолей Цикл
		...
		Тогда
			 		ТекстПоляЗапроса = ТекстПоляЗапроса + ",
		|			ВЫБОР КОГДА РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) И РаботникиОрганизаций.ПериодЗавершения <= &ДатаАктуальности Тогда " + ОписаниеПоля +"Завершения Иначе " + ОписаниеПоля + " КОНЕЦ КАК " + ПсевдонимПоля;	
		ИначеЕсли ОписаниеПоля = "РаботникиОрганизаций.ПодразделениеОрганизацииЗавершения"  
							ИЛИ  ОписаниеПоля = "РаботникиОрганизаций.ДолжностьЗавершения"
							ИЛИ  ОписаниеПоля = "РаботникиОрганизаций.ЗанимаемыхСтавокЗавершения"
							ИЛИ  ОписаниеПоля = "РаботникиОрганизаций.ГрафикРаботыЗавершения"
		...
		Тогда
			 		ТекстПоляЗапроса = ТекстПоляЗапроса + ",
		|			ВЫБОР КОГДА РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) И РаботникиОрганизаций.ПериодЗавершения <= &ДатаАктуальности Тогда " + ОписаниеПоля +" Иначе " + СтрЗаменить(ОписаниеПоля,"Завершения","") + " КОНЕЦ КАК " + ПсевдонимПоля;	
        Иначе
		ТекстПоляЗапроса = ТекстПоляЗапроса + ",
...
Функция СформироватьЗапрос() Экспорт
	ДобавитьПараметрЗапроса(мТаблицаПараметрыЗапроса, "ДатаАктуальности",	КонецДня(ДатаАктуальности));
