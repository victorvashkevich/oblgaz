...
Процедура СформироватьЗапросЗначенийПоказателейУправленческогоУчета(Запрос, ПоВременнойСхемеМотивации = Ложь) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ЗначенияПоказателей." + ОбщегоНазначения.ВалютаПоказателяИмяПоляЗапроса() + " = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|			ТОГДА ВЫБОР
	...
	|				КОНЕЦ
	|		ИНАЧЕ ЗначенияПоказателей." + ОбщегоНазначения.ВалютаПоказателяИмяПоляЗапроса() + "
	|	КОНЕЦ КАК Валюта
