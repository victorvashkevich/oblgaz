...
//vvv
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ДОСТУПНОСТИ НАЧИСЛЕНИЙ В КАДРОВЫХ ДОКУМЕНТАХ
//vvv
Функция ДоступнаОднаИзРолей(ИменаРолей)
	
	Если НЕ ПараметрыСеанса.ИспользоватьОграниченияПравДоступаНаУровнеЗаписей или РольДоступна("ПолныеПрава") Тогда
		Возврат Истина;
	КонецЕсли;
	
	Роли = ОбщегоНазначенияЗК.РазложитьСтрокуВМассивПодстрок(ИменаРолей);
	ДоступнаОднаИзРолей = Ложь;
	Для Каждого ИмяРоли Из Роли Цикл
		ДоступнаОднаИзРолей = ДоступнаОднаИзРолей или РольДоступна(ИмяРоли);
	КонецЦикла;
	
	Возврат ДоступнаОднаИзРолей;
	
КонецФункции // ДоступнаОднаИзРолей
//vvv
Функция ДоступныНачисленияСотрудников(Сотрудники, ИменаРазрешающихРолей) Экспорт
	
	//vvv пока сделаем так 
	Возврат Истина;
	// управление сведениями о начислениях доступно только при одной из разрешающих ролей...
	Если ДоступнаОднаИзРолей(ИменаРазрешающихРолей) Тогда
		Возврат Истина;
	КонецЕсли;
	
	// ...или при условии, что в документе нет сотрудников, для которых недоступны начисления
	ПросмотрНачисленийДоступенДляВсехСотрудников = ЗаполнениеНачислениямиПолныеПрава.ПросмотрНачисленийКадровикамиДоступенДляВсехСотрудников(Сотрудники, ОбщегоНазначения.ПолучитьЗначениеПеременной("глТекущийПользователь"));
	
	Возврат ПросмотрНачисленийДоступенДляВсехСотрудников;
	
КонецФункции // ДоступныНачисленияСотрудников