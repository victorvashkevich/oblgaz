...
Процедура ПроверитьКорректностьКурсаНа01_01_1980(Валюта) Экспорт
	...
	Если (СтруктураКурса.Курс = 0) Или (СтруктураКурса.Кратность = 0) Тогда
		...
		НаборКурсыВалют.Отбор.Валюта.Установить(Валюта);
		НоваяЗапись = НаборКурсыВалют.Добавить();
		НоваяЗапись.Период    = ДатаКурса;
		НоваяЗапись.Валюта    = Валюта;
		НоваяЗапись.Курс      = 1;
		НоваяЗапись.Кратность = 1;
		НаборКурсыВалют.ОбменДанными.Загрузка = Истина;
		НаборКурсыВалют.Записать(Истина);
		
		НаборКурсыВалют = РегистрыСведений.КурсыВалютДляРасчетовСПерсоналом.СоздатьНаборЗаписей();
		НаборКурсыВалют.Отбор.Валюта.Установить(Валюта);
		НоваяЗапись = НаборКурсыВалют.Добавить();
		НоваяЗапись.Период    = ДатаКурса;
		НоваяЗапись.Валюта    = Валюта;
