...
//vvv добавим последний параметр, чтобы знать куда прикреплять групировку, т.к. по умолчанию берется последний уровень
Функция ДобавитьГруппировку(КомпоновщикНастроек, Знач Поле = Неопределено, Строки = Истина, Родитель = Неопределено) Экспорт
