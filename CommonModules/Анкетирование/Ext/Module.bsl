...
Процедура ЗаполнитьКонтактнуюИнформациюПоОпросу(ФизическоеЛицо, Опрос, Вопрос, Раздел, ВидКИ, ТипКИ)
	...
	Если	ТипКИ = Перечисления.ТипыКонтактнойИнформации.Адрес	тогда
		...
		МассивПолей	=	ОбщегоНазначенияЗК.РазложитьСтрокуВМассивПодстрок(СтрокаСОтветом.ТиповойОтвет,	"¤");
...
Функция ПолучитьПредставлениеКИ(ТипКИ, ЗначениеКИ) Экспорт
	...
	Если ТипКИ = Перечисления.ТипыКонтактнойИнформации.Адрес тогда
		МассивПолей = ОбщегоНазначенияЗК.РазложитьСтрокуВМассивПодстрок(ЗначениеКИ, "¤");
...
Функция ПолучитьОтветы(Выборка, Тип, ФлагКолонки = Ложь)
	...
	Иначе
		...
		Иначе
			...
			Иначе
				ОбщегоНазначенияЗК.СообщитьОбОшибке("Справочник """+Объект.Синоним+""" не имеет ни одного элемента. Рекомендуется заполнить его и сформировать анкету еще раз.");
...
Функция ПолучитьТипЗначенияОтвета(Выборка, ФлагКолонки)
	...
	Если Выборка[Префикс + "ТипОтветаНаВопрос"]  = Перечисления.ТипыОтветаНаВопросАнкеты.КонтактнаяИнформация Тогда
		СтрокаТипКИ = ОбщегоНазначенияЗК.ПолучитьИмяЭлементаПеречисленияПоЗначению(Выборка[Префикс + "ТипКонтактнойИнформации"]);
		СтрокаВидКИ = ОбщегоНазначенияЗК.ПолучитьИмяЭлементаПеречисленияПоЗначению(Выборка[Префикс + "ВидОбъектаКонтактнойИнформации"]);
...
Функция ПолучитьИмяФайлаАнкеты(ТиповаяАнкета) Экспорт
	...
	КодАнкеты = ОбщегоНазначенияЗК.ПолучитьНомерНаПечать(Выборка);
...
Процедура СоздатьОпрос(УчетнаяЗапись, ТекущийПредмет) Экспорт
	
	Если Не УправлениеЭлектроннойПочтой.ПолучитьЗначениеРеквизитаУчетнойЗаписи(УчетнаяЗапись, "ИспользоватьКлассификациюПисемПоПредметам") Тогда
...
Процедура ВставитьКнопкуСоздатьОпросВКоманднуюПанель(КоманднаяПанель, ОбработчикКоманды)
	...
	Для Каждого Кнопка Из Кнопки Цикл
		Если Кнопка.ТипКнопки = ТипКнопкиКоманднойПанели.Разделитель Тогда
			МестоВставки = Кнопки.Индекс(Кнопка);
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьКомандыВФормуЭлектронногоПисьма(ФормаДокумента, ДополнительныеДействия) Экспорт
...
Процедура ВыполнитьКомандыФормыЭлектронногоПисьма(ИмяКоманды, ФормаДокумента) Экспорт
	
	Если ИмяКоманды = "СоздатьОпрос" Тогда
