...
Функция СтруктураНастроекОбменаЧерезВнешнееСоединение(Структура) Экспорт
	...
	СтруктураНастроекОбменаВнешнееСоединение.Вставить("ВидОбмена", ВидОбмена(УзелИнформационнойБазы));
...
Процедура ЗаписатьСообщениеСИзменениямиДляУзла(СтруктураНастроекОбмена, Знач ИмяФайлаСообщенияОбмена = "", СообщениеОбмена = "")
	...
	Если СтруктураНастроекОбмена.ВидОбмена = "РаспределеннаяИнформационнаяБаза" Тогда
	...
	ИначеЕсли СтруктураНастроекОбмена.ВидОбмена = "ПравилаКонвертации" Тогда // Универсальный обмен (обмен по правилам конвертации)
	...
	ИначеЕсли СтруктураНастроекОбмена.ВидОбмена = "XMLСхемаЭтойКонфигурации" Тогда
		
		Отказ = Ложь;
		КоличествоОбъектовОбработано = 0;
		
		ВыполнитьСтандартнуюВыгрузкуИзмененийДляУзла(
			Отказ,
			СтруктураНастроекОбмена.УзелИнформационнойБазы,
			ИмяФайлаСообщенияОбмена,
			СообщениеОбмена,
	...
	ИначеЕсли СтруктураНастроекОбмена.ВидОбмена = "Произвольный" Тогда
		...
		Если СтандартнаяОбработка = Истина Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не определен алгоритм обмена данными для плана обмена <%1>'"), СтруктураНастроекОбмена.ИмяПланаОбмена
				);
		КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неизвестный вид обмена данными <%1>.'"), СтруктураНастроекОбмена.ВидОбмена
		);
	КонецЕсли;
...
Процедура ПрочитатьСообщениеСИзменениямиДляУзла(СтруктураНастроекОбмена, Знач ИмяФайлаСообщенияОбмена = "", СообщениеОбмена = "", Знач ТолькоПараметры = Ложь)
	...
	Если СтруктураНастроекОбмена.ВидОбмена = "РаспределеннаяИнформационнаяБаза" Тогда // Обмен в РИБ
	...
	ИначеЕсли СтруктураНастроекОбмена.ВидОбмена = "ПравилаКонвертации" Тогда
...
Процедура ЗафиксироватьЗавершениеОбменаСОшибкой(Знач УзелИнформационнойБазы, 
	...
	СтруктураНастроекОбмена.Вставить("ВидОбмена", ВидОбмена(УзелИнформационнойБазы));
...
Функция ПолучитьСтруктуруНастроекОбменаДляУзлаИнформационнойБазы(
	...
	СтруктураНастроекОбмена.ВидОбмена              = ВидОбмена(УзелИнформационнойБазы);
...
Процедура ВыполнитьИнициализациюСтруктурыНастроекОбмена(СтруктураНастроекОбмена, НастройкаВыполненияОбмена, НомерСтроки)
	...
	СтруктураНастроекОбмена.ВидОбмена = ВидОбмена(СтруктураНастроекОбмена.УзелИнформационнойБазы);
...
Функция СтруктураНастроекОбменаБазовая()
	...
	СтруктураНастроекОбмена.Вставить("ВидОбмена", "");
...
// Возвращает вид обмена данными для переданного значения узла.
//
//  Параметры:
// УзелИнформационнойБазы - ПланОбменаСсылка - ссылка на узел плана обмена, для которого необходимо получить значение функции.
//
//  Возвращаемое значение:
// Тип: Строка. Строковой идентификатор вида обмена данными.
//
Функция ВидОбмена(Знач УзелИнформационнойБазы) Экспорт
	
	ИмяПланаОбмена = ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(УзелИнформационнойБазы);
	
	Результат = ПланыОбмена[ИмяПланаОбмена].ВидОбмена();
	
	Если ПустаяСтрока(Результат) Тогда
		
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для плана обмена <%1> не задан вид обмена данными.'"), ИмяПланаОбмена
		);
	ИначеЕсли (Результат = "РаспределеннаяИнформационнаяБаза"
		И Не ОбменДаннымиПовтИсп.ЭтоУзелРаспределеннойИнформационнойБазы(УзелИнформационнойБазы))
		ИЛИ
		(Результат <> "РаспределеннаяИнформационнаяБаза"
		И ОбменДаннымиПовтИсп.ЭтоУзелРаспределеннойИнформационнойБазы(УзелИнформационнойБазы)) Тогда
		
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Рассогласование признака <Распределенная ИБ> и вида обмена данными <%1> для плана обмена <%2>'"), Результат, ИмяПланаОбмена
		);
	КонецЕсли;
