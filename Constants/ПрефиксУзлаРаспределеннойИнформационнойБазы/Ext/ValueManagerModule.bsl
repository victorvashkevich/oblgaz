...
Процедура ПередЗаписью(Отказ)
	Если ЭтотОбъект.Значение <> Константы[ЭтотОбъект.Метаданные().Имя].Получить() Тогда
		Если ОбщегоНазначения.ОпределитьТекущийРежимРаботыМонопольный() Тогда
			ПолныеПрава.ОпределитьФактИспользованияРИБ();
		Иначе
			ВызватьИсключение "Изменение префикса узла РИБ возможно только в монопольном режиме";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
