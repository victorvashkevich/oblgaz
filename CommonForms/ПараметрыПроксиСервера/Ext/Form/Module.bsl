	...
	Если ЗакрытьФорму Тогда
		...
		Если ТребуетсяВыполнитьПопыткуЗаписиНастроекВФайл Тогда
			...
			Если ЭтоФайловаяИБ И НастройкаПроксиНаКлиенте Тогда
				ТекстВопроса = НСтр("ru = 'Возможно применить настройки параметров прокси-сервера для механизмов, расчитанных на работу с веб-сервисами.
...
Функция ЗаписатьНастройкиПроксиВДействующийФайлКонфигурацииСети(знач ПутьККонфФайлу = "")
	...
	Если ВариантИспользованияПроксиСервера = 1 Тогда
		...
		Если ЭтоФайловаяИБ Тогда
			ТекстСообщения = НСтр("ru = 'Файл ""%1"" удален на этом компьюетере.'");
		Иначе
	...
	Иначе
		...
		Попытка
			...
			Если ЭтоФайловаяИБ Тогда
				ТекстСообщения = НСтр("ru = 'Настройки прокси-сервера записаны в файл ""%1"" на этом компьюетере.'");
			Иначе
