...
Процедура СформироватьОсновнуюТаблицуСвойствОтчетов() Экспорт
	...
	Для Каждого Обл Из КонстантаНазвОбластивМакете Цикл
		...
		Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
			...
			//	СтрокаУровня1.Шаг = "3";
			//Иначе
				СтрокаУровня1.Шаг = ОпределитьПризнакиПериодов(ЭталонныеИсточники.Область(Ном, 8).Текст);
			//КонецЕсли;
			СтрокаУровня1.Нарастающий = ОпределитьПризнакиПериодичности(ЭталонныеИсточники.Область(Ном, 8).Текст);
...
// Параметры:
//  ПериодыКолонка8 - строка из макета СписокОтчетов обработки ОбновлениеРегламентированнойОтчетности
...
//                 5 - год;
//
Функция ОпределитьПризнакиПериодов(ПериодыКолонка8)

	Если (Найти(ПериодыКолонка8, "Месяц") > 0)
	   И (Найти(ПериодыКолонка8, "Квартал") > 0) Тогда
		Возврат "3";
	ИначеЕсли (Найти(ПериодыКолонка8, "Месяц") > 0)
		    И (Найти(ПериодыКолонка8, "Квартал") = 0) Тогда
		Возврат "2";
	ИначеЕсли (Найти(ПериодыКолонка8, "Квартал") > 0)
		    И (Найти(ПериодыКолонка8, "Месяц") = 0) Тогда
		Возврат "1";
	ИначеЕсли (Найти(ПериодыКолонка8, "Полугодие") > 0) Тогда
		Возврат "4";
	ИначеЕсли (Найти(ПериодыКолонка8, "Год:") > 0) Тогда
		Возврат "5";
	КонецЕсли;
			...
			Пока СтруктураДанных.Свойство(ИмяПоказателя) И ВыполнятьЦикл Цикл
				...
				Иначе
					Если ВариантУказанияИНН = "0" Тогда
						// Только ЮР
						ИННВерный = РегламентированнаяОтчетность.ИННСоответствуетТребованиям(ЗначениеПоказателя, Перечисления.ЮрФизЛицо.ЮрЛицо);
					ИначеЕсли ВариантУказанияИНН = "1" Тогда
						// Только Физ.
						ИННВерный = РегламентированнаяОтчетность.ИННСоответствуетТребованиям(ЗначениеПоказателя, Перечисления.ЮрФизЛицо.ФизЛицо);
...
Функция ПолучитьЗначениеИзДокумента(Знач Формула, Знач БазовыйПериод, Организация, ИМНС = Неопределено, ВыбраннаяФорма = Неопределено) Экспорт
	...
	Если РезультатПоиска.Количество() <> 1 Тогда
		...
		ТаблицаПодходящихДокументов = ПолучитьСохраненныеДокументы(ИсточникОтчета, Организация, ТаблицаФильтрПериод, БазовыйПериод, Истина, ФильтрПоОтчету, ИМНС, ВыбраннаяФорма);
	...
	Для Инд = 1 По СтрЧислоВхождений(ФормулаСтандарт, " ") + 1 Цикл
		...
		Если Найти(Часть, ".") > 0 Тогда
			...
			Иначе
				...
				// в ключенным флагом - Проверки КС.
...
// Функция округляет число согласно установленной размерности чисел.
// Функция применяется в декларации по прибыли. Округляет без деления, учитывая точность ед. имзерения в декларации.
// Параметры:
//  ИсходноеЧисло - заданное число;
//
// Возвращаемое значение:
