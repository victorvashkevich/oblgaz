...
Перем ПолеОбъектыДляОтложеннойЗаписи; // Таблица значений для запись объектов ссылочных типов после загрузки данных
...
Перем ПолеИспользуетсяВерсионирование;
...
Функция ДокументыДляОтложенногоПроведения()
	...
	Если ТипЗнч(ПолеДокументыДляОтложенногоПроведения) <> Тип("ТаблицаЗначений") Тогда
		...
		ПолеДокументыДляОтложенногоПроведения.Колонки.Добавить("ЭтоКоллизия", одОписаниеТипа("Число"));
...
Функция ОбъектыДляОтложеннойЗаписи()
	...
	Если ТипЗнч(ПолеОбъектыДляОтложеннойЗаписи) <> Тип("ТаблицаЗначений") Тогда
		
		// инициализируем таблицу для отложенной записи объектов
		ПолеОбъектыДляОтложеннойЗаписи = Новый ТаблицаЗначений;
		ПолеОбъектыДляОтложеннойЗаписи.Колонки.Добавить("ОбъектСсылка");
		ПолеОбъектыДляОтложеннойЗаписи.Колонки.Добавить("ЭтоКоллизия", одОписаниеТипа("Число"));
		ПолеОбъектыДляОтложеннойЗаписи.Колонки.Добавить("ОбъектЗаписанУспешно", одОписаниеТипа("Булево"));
...
// Функция-свойство: флаг использования версионирования
//
// Тип: Булево
//
Функция ИспользуетсяВерсионирование()
	
	Если ТипЗнч(ПолеИспользуетсяВерсионирование) <> Тип("Булево") Тогда
		
		ПолеИспользуетсяВерсионирование = Ложь;
		ОбменДаннымиСервер.ПриОпределенииИспользованияВерсионирования(ПолеИспользуетсяВерсионирование);
		
	КонецЕсли;
	
	Возврат ПолеИспользуетсяВерсионирование;
...
// 
Процедура ЗаписатьОбъектВИБ(Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Отказ = Ложь;
	СообщениеОбОшибке = "";
	ОбменДаннымиСервер.ПередЗаписьюДанныхИзСообщенияОбмена(
		Объект, УзелОбменаЗагрузкаДанных, Отказ, СообщениеОбОшибке
	);
	
	Если Отказ Тогда
		
		// Если загрузка запрещена регистрируется ошибка и загрузка продолжается.
		ЗП = ПолучитьСтруктуруЗаписиПротокола(84, "{}: " + Строка(СообщениеОбОшибке));
		ЗП.Объект = Объект;
		ЗП.ТипОбъекта = Тип;
		...
		Если ИспользуетсяВерсионирование() И (ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъекта) Или ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта)) Тогда
			
			Если Объект.Ссылка.Пустая() Тогда
				СтрокаСообщенияОбОшибке = Строка(Объект) + НСтр("ru = ' запрещено загружать в запрещенный период.'") + Символы.ПС + Символы.ПС + Строка(СообщениеОбОшибке);
			Иначе
				ОбъектВБазе = Объект.Ссылка.ПолучитьОбъект();
				ОбменДаннымиСервер.ПриСозданииВерсииОбъекта(ОбъектВБазе, , Перечисления["ТипыВерсийОбъекта"].ИзмененоПользователем,,,Истина);
				СтрокаСообщенияОбОшибке = Строка(СообщениеОбОшибке);
			КонецЕсли;
			
			ОбменДаннымиСервер.ПриСозданииВерсииОбъекта(Объект, УзелОбменаЗагрузкаДанных,Перечисления["ТипыВерсийОбъекта"].НепринятыеДанныеПоДатеЗапретаОбъектСуществует,
				СтрокаСообщенияОбОшибке, УникальныйИдентификаторСтрокой, Истина);
...
//	Тип - Строка - Тип объекта строкой.
//
Процедура ОтменитьПроведениеОбъектаВИБ(Объект, Тип, ЗаписатьОбъект = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Параметр" И ТипУзла = ТипУзлаXMLНачалоЭлемента Тогда
			...
			Если ИспользуетсяПриЗагрузке
				//vvv переделаем условие, т.к. из-за этого реквизита не передавались параметры в приемник
				//возможно где-то вылезет
				//И РежимОбмена = "Выгрузка"
				И РежимОбмена <> "Загрузка"
				Тогда
				//
...
Функция ПрочитатьНаборЗаписейРегистра()
	...
	Если Объект <> Неопределено тогда
		...
		ЕстьКоллизия = ПроверитьНаличиеКоллизийДляОбъекта(Объект, "РегистрСведений", ПриоритетОбъектаОбмена, ИмяРегистра);
...
Функция ПроверитьНаличиеКоллизийДляОбъекта(Объект, ИмяТипаОбъекта, ПриоритетОбъектаОбмена,
	ДополнительнаяИнформация = "", УникальныйИдентификаторСтрокой = "")
	...
	Если ЕстьКоллизия Тогда
		...
		ЗаписатьВПротоколВыполнения(КСообщенияОбОшибках, ЗП, Ложь);
		...
		Если ИспользуетсяВерсионирование() И (ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъекта) Или ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта)) Тогда
			...
			Если ПриоритетОбъектаОбмена <> Перечисления.ПриоритетыОбъектовОбмена.ПриоритетПоступившегоПриОбменеОбъектаВыше Тогда
				...
				Комментарий = НСтр("ru = 'Непринятые данные (автоматическое разрешение коллизий).'");
				ОбменДаннымиСервер.ПриСозданииВерсииОбъекта(Объект, УзелОбменаЗагрузкаДанных,
					Перечисления["ТипыВерсийОбъекта"].НепринятыеДанныеПоКоллизии, Комментарий, УникальныйИдентификаторСтрокой, Истина);
...
Функция ПрочитатьОбъект(УникальныйИдентификаторСтрокой = "")
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = ТипУзлаXMLКонецЭлемента) Тогда
			...
			СтандартныеПодсистемыСервер.ПриПолученииДанныхОтПодчиненного(Объект, ПолучениеЭлемента, ОтправкаНазад);
			...
			Если ИмяТипаОбъекта = "Документ" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Попытка
						...
						ЕстьКоллизия = ПроверитьНаличиеКоллизийДляОбъекта(Объект, ТипОбъекта, ПриоритетОбъектаОбмена, ИмяКонстанты, УникальныйИдентификаторСтрокой);
						
						ЗаписатьОбъект = Истина;
						КоллизияПринята = Ложь;
						...
						Если ЗаписатьОбъект Тогда
							...
							Если РежимЗаписи = РежимЗаписиДокумента.Запись Тогда
								...
								ЗаписатьОбъектВИБ(Объект, ТипОбъекта,, ОтправкаНазад, УникальныйИдентификаторСтрокой);
							...
							ИначеЕсли РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
								...
								ОтменитьПроведениеОбъектаВИБ(Объект, ТипОбъекта, ЗаписатьОбъект, УникальныйИдентификаторСтрокой);
								...
								Если ЗаписатьОбъект
									...
									СтрокаТаблицы.ЭтоКоллизия = КоллизияПринята;
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					ЕстьКоллизия = ПроверитьНаличиеКоллизийДляОбъекта(Объект, ИмяТипаОбъекта, ПриоритетОбъектаОбмена, ИмяКонстанты, УникальныйИдентификаторСтрокой);
					...
					Если ЗаписатьОбъект Тогда
						...
						Если ЗаписатьОбъект
							...
							СтрокаТаблицы = ОбъектыДляОтложеннойЗаписи().Добавить();
							СтрокаТаблицы.ОбъектСсылка = Объект.Ссылка;
							СтрокаТаблицы.ЭтоКоллизия = КоллизияПринята;
...
Процедура ЗаполнитьТаблицуРеквизитовРегистрацииОбъектовПоПравилу(ПКО, ТаблицаРезультат)
	...
	Для Каждого ПКГС Из МассивПКГС Цикл
		...
		//vvv
		Если ПКГС.ВидПриемника = "НаборДвиженийРегистраСведений" ТОгда Продолжить; КонецЕсли;
		//
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если    ИмяУзла = "Объект"
			...
			Иначе
				...
				СтрокаТаблицы.ЭтоКлассификатор                    = ПКО.СинхронизироватьПоИдентификатору И ПКО.ПродолжитьПоискПоПолямПоискаЕслиПоИдентификаторуНеНашли;
				СтрокаТаблицы.ЭтоУдалениеОбъекта = Ложь;
...
Процедура ПрочитатьДанныеПоОбмену()
	...
	Если РежимЗагрузкиДанныхВИнформационнуюБазу()
		И УзелОбменаЗагрузкаДанных.НомерПринятого >= НомерСообщения() Тогда
	...
	Если РежимЗагрузкиДанныхВИнформационнуюБазу() И УдалитьРегистрациюИзменений Тогда
...
Процедура ВыполнитьОтложеннуюЗаписьОбъектов()
	...
	// Сворачиваем таблицу по уникальным полям
	ОбъектыДляОтложеннойЗаписи().Свернуть("ОбъектСсылка, ОбъектЗаписанУспешно", "ЭтоКоллизия");
	
	// Инициализируем колонку результата записи объекта
	ОбъектыДляОтложеннойЗаписи().ЗаполнитьЗначения(Ложь, "ОбъектЗаписанУспешно");
	
	Для Каждого СтрокаТаблицы Из ОбъектыДляОтложеннойЗаписи() Цикл
		
		Если СтрокаТаблицы.ОбъектСсылка.Пустая() Тогда
			Продолжить;
		КонецЕсли;
		
		Объект = СтрокаТаблицы.ОбъектСсылка.ПолучитьОбъект();
		...
		Попытка
			...
			Если Объект.ПроверитьЗаполнение() Тогда
				...
				СтрокаТаблицы.ОбъектЗаписанУспешно = Истина;
				
			Иначе
				
				СтрокаТаблицы.ОбъектЗаписанУспешно = Ложь;
		...
		// если документ провести не удалось фиксируем ошибку в ЖР
		Если Не СтрокаТаблицы.ОбъектЗаписанУспешно Тогда
			
			СтрокаСообщения = НСтр("ru = 'Ошибка при отложенной запись объекта: %1. Описание ошибки: %2'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Строка(Объект), ОписаниеОшибки);
			ЗаписьЖурналаРегистрацииОбменДанными(СтрокаСообщения, УровеньЖурналаРегистрации.Предупреждение);
			...
			РегистрыСведений.РезультатыОбменаДанными.ЗарегистрироватьОшибкуПроверкиОбъекта(СтрокаТаблицы.ОбъектСсылка, УзелОбменаЗагрузкаДанных,
				Объект.Метаданные().Синоним, ТекстОшибки, Перечисления.ТипыПроблемОбменаДанными.НезаполненныеРеквизиты);
...
Функция ПоляПоискаМеханизмаСопоставленияОбъектов(Знач ПоляПоиска)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(ПоляПоискаКоллекция);
КонецФункции
...
// Возвращает интерфейс обработчика во внешней обработке 
//
// Параметры:
...
Процедура ВыполнитьОбработчик_ПКС_ПередВыгрузкойСвойства(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	...
	| Отказ, Значение, ТипПриемника, ИмяПКО, ИмяПКОВидСубконто, Пусто, Выражение, УзелКоллекцииСвойств, НеЗамещать, ВыгрузитьОбъект";
...
Процедура ВыполнитьОбработчик_ПКС_ПриВыгрузкеСвойства(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные,
	...
	| ИмяПКО, ПКОСвойств,УзелСвойства, УзелКоллекцииСвойств,ИмяПКОВидСубконто, ВыгрузитьОбъект";
