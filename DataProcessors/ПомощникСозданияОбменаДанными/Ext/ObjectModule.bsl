...
// Выполняет действия по настройке нового обмена в Сервисе.
//
	...
	Попытка
		...
		Если ВерсияКорреспондента_2_1_1_7 Тогда
			...
			WSПрокси.CreateExchange(ИмяПланаОбмена, 
							...
							Сериализатор.ЗаписатьXDTO(ЭкранироватьПеречисления(НастройкаОтборовНаУзле)),
...
Процедура СоздатьОбновитьУзлыПланаОбмена(НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле, КодЭтогоУзла, КодНовогоУзла)
	...
	СтандартныеПодсистемыСервер.ПроверитьСоставПланаОбмена(ИмяПланаОбмена);
	...
	НовыйУзел.ДополнительныеСвойства.Вставить("Загрузка");
	НовыйУзел.Записать();
