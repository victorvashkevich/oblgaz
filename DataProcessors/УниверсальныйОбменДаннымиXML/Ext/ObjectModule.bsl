...
//  не выгружается, т.к. код обработчика находится не в узле правил обмена, а в отдельном узеле.
...
//  Ключ элемента структуры - имя алгоритма, значение - интрефейс вызова процедуры, содержащей код алгоритма.
//
// Параметры:
//  Результат    - Объект типа ЗаписьТекста - для вывода обработчиков в текстовый файл.
// 
...
//  Правило        - строка таблицы занчений с правилами конвертации объекта
...
//  АлгоритмыОбработчика - Массив- содержит имена алгоритмов, которые вызыватся из заданного обработчика
//  
...
// Функция возвращет измененный код алгоритма с учетом вложенных алгоритмов. Вместо оператора вызова
...
//  АлгоритмВладелец           - Строка - имя алгоритма, являющегося родительстким по отношению 
...
// Возвращает массив с именами алгоритмов, которые были обныружены в теле переданного обработчика
//
// Параметры:
...
//
// Параметры:
//  Правило            - произвольная коллекия значений - ПКО, ПВД, ПОД
...
// Дополняет имеющеся коллекции с правилами обмена интерфейсами вызова обработчиков
//
// Параметры:
//  СтруктураКонвертация - Структура - содержит правила конвертации и глобальные обработчики
//  ТаблицаПКО           - ТаблицаЗначений - содержит парвила конвертации объектов
...
Функция ОпределитьПоТипуОбъектаИспользоватьДополнительныйПоискПоПолямПоиска(ТипСсылкиСтрокой)
	...
	ЗначениеСоотвестствия = мСоответствиеДопПараметровПоиска.Получить(ТипСсылкиСтрокой);
...
Функция ОпределитьПоТипуОбъектаАлгоритмПоследовательностиПолейПоиска(ТипСсылкиСтрокой)
	...
	ЗначениеСоотвестствия = мСоответствиеАлгоритмовПоискаЭлементов.Получить(ТипСсылкиСтрокой);
...
Процедура ДополнитьСтекНеЗаписанныхОбъектов(Нпп, ГНпп, Объект, ИзвеснаяСсылка, ТипОбъекта, ПараметрыОбъекта)
	...
	мГлобальныйСтекНеЗаписанныхОбъектов.Вставить(НомерДляСтека, Новый Структура("Объект, ИзвеснаяСсылка, ТипОбъекта, ПараметрыОбъекта", Объект, ИзвеснаяСсылка, ТипОбъекта, ПараметрыОбъекта));
...
Функция ОбработатьВыгрузкуДляПлановОбмена(СоответствиеУзловИПравилВыгрузки, СтруктураДляУдаленияРегистрацииИзменений)
	...
	Для Каждого СтрокаСоотвествия Из СоответствиеУзловИПравилВыгрузки Цикл
