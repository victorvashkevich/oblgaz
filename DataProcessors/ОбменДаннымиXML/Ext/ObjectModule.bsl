...
Функция НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать,
	...
	Если РежимПоискаОсновногоОбъекта Тогда
		...
		Если СвойстваОбъектаМодифицированы
			...
			Если ОбъектНайден
				И НеЗамещатьОбъектСозданныйВИнформационнойБазеПриемнике Тогда
				
				ОпределитьОбъектБылСозданВТекущейИнформационнойБазе(Объект, ОбъектБылСозданВТекущейИнформационнойБазе);
				Если ОбъектБылСозданВТекущейИнформационнойБазе <>  Истина Тогда
...
Процедура ОбработатьУстановкуСвойствПоискаУОбъекта(УстанавливатьУОбъектаВсеСвойстваПоиска, ТипОбъекта, СвойстваПоиска, 
	...
	Если ИзмененыРеквизитыОбъекта
		...
		Если СсылкаНаОбъект <> Неопределено
			...
			ОпределитьОбъектБылСозданВТекущейИнформационнойБазе(СсылкаНаОбъект, ОбъектБылСозданВТекущейИнформационнойБазе);
			Если ОбъектБылСозданВТекущейИнформационнойБазе =  Истина Тогда
...
Процедура ОпределитьОбъектБылСозданВТекущейИнформационнойБазе(СсылкаНаОбъект, ОбъектБылСозданВТекущейИнформационнойБазе)
	
	Если ОбъектБылСозданВТекущейИнформационнойБазе <> Неопределено Тогда
		
		Возврат;
		
	КонецЕсли;
	
	Если мИспользоватьИнформациюОМестеСозданияОбъектовПриЗагрузке = Ложь Тогда
		
		ОбъектБылСозданВТекущейИнформационнойБазе = Ложь;	
		Возврат;
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		...
		ОбъектБылСозданВТекущейИнформационнойБазе = Истина;
		Возврат;
		
	КонецЕсли;
	
	// если записи в регистре нет, то только в этом случае объект был создан в этой ИБ
	ОбъектБылСозданВТекущейИнформационнойБазе = Ложь;
		...
		ИначеЕсли ИмяУзла = "Ссылка" Тогда
			...
			Иначе
				...
				ИначеЕсли ОбъектНайден Тогда
					...
					ИначеЕсли НеЗамещатьОбъектСозданныйВИнформационнойБазеПриемнике И (Не ЕстьОбработчикПриЗагрузке) Тогда
						
						ОпределитьОбъектБылСозданВТекущейИнформационнойБазе(Ссылка, ОбъектБылСозданВТекущейИнформационнойБазе);
						Если ОбъектБылСозданВТекущейИнформационнойБазе =  Истина Тогда
			...
			Если ЕстьОбработчикПриЗагрузке Тогда
				...
				Если ОбъектНайден Тогда
					...
					ИначеЕсли НеЗамещатьОбъектСозданныйВИнформационнойБазеПриемнике Тогда
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
						   ...
						   НеИспользоватьПравилоСГлобальновВыгрузкойИНеЗапоминатьВыгруженные = Ложь)
