...
Процедура ПечатьСМ(ТаблицаВидовРасчета, ВидОрганизационнойСтруктурыПредприятия) Экспорт
	...
	Для Каждого СтрокаВидаРасчета Из ТаблицаВидовРасчета Цикл
		...
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаДо3ЛетНаВторого Тогда
			...
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие по уходу на ребенком до 3х лет на второго и последующих детей", СтрокаЗначения, ТекущийРаздел);
			
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаДо3ЛетНаРебенкаИнвалида Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие по уходу на ребенком до 3х лет на ребенка-инвалида", СтрокаЗначения, ТекущийРаздел);
		...
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаСтарше3ЛетНаРебенкаИнвалида Тогда
			...
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие на ребенка старще 3х лет на ребенка-инвалида", СтрокаЗначения, ТекущийРаздел);
