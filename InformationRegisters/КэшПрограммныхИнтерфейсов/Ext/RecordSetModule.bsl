...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Если ТребуетсяКонтроль Тогда
		...
		Если Не ДополнительныеСвойства.Свойство("ПараметрыПолучения", ПараметрыПолучения) Тогда
			...
			ВызватьИсключение НСтр("ru = 'При записи данных в регистр сведений КэшПрограммныхИнтерфейсов из сеансов с включенным разделением требуется передавать параметры получения данных для кэша интерфейсов в дополнительном свойстве набора записей ПараметрыПолучения!'");
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если ТребуетсяКонтроль Тогда
		ЭтотОбъект.Загрузить(ТаблицаКонтроля);
	КонецЕсли;
