...
Функция ВерсияФорматаОбмена(Знач УзелИнформационнойБазы) Экспорт
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ОбщиеНастройкиУзловИнформационныхБаз.ВерсияФорматаОбмена КАК ВерсияФорматаОбмена
	|ИЗ
	|	РегистрСведений.ОбщиеНастройкиУзловИнформационныхБаз КАК ОбщиеНастройкиУзловИнформационныхБаз
	|ГДЕ
	|	ОбщиеНастройкиУзловИнформационныхБаз.УзелИнформационнойБазы = &УзелИнформационнойБазы";
	...
	Возврат ?(ПустаяСтрока(Выборка.ВерсияФорматаОбмена), Неопределено, Выборка.ВерсияФорматаОбмена);
КонецФункции

Процедура УстановитьВерсиюФорматаОбмена(Знач УзелИнформационнойБазы, Знач Версия) Экспорт
	...
	Попытка
		...
		|ГДЕ
		|	ОбщиеНастройкиУзловИнформационныхБаз.УзелИнформационнойБазы = &УзелИнформационнойБазы
		|	И ОбщиеНастройкиУзловИнформационныхБаз.ВерсияФорматаОбмена = &ВерсияФорматаОбмена
		|";
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("УзелИнформационнойБазы", УзелИнформационнойБазы);
		Запрос.УстановитьПараметр("ВерсияФорматаОбмена", Версия);
		Запрос.Текст = ТекстЗапроса;
		...
		Если Запрос.Выполнить().Пустой() Тогда
			...
			СтруктураЗаписи = Новый Структура;
			СтруктураЗаписи.Вставить("УзелИнформационнойБазы", УзелИнформационнойБазы);
			СтруктураЗаписи.Вставить("ВерсияФорматаОбмена", Версия);
			ОбновитьЗапись(СтруктураЗаписи);
		КонецЕсли;
