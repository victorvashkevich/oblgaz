...
Процедура Автозаполнение() Экспорт
	...
	ПрошлыйПериодПланирования = Бюджетирование.ДатаНачалаПериода(Бюджетирование.ДобавитьИнтервал(ДатаПланирования, ПериодичностьПланирования, -1), ПериодичностьПланирования);
	...
	ТекущийКурс				= ОбщегоНазначения.ПолучитьКурсВалюты(ВалютаУчета, ПрошлыйПериодПланирования);
	...
	Запрос.УстановитьПараметр("ДатаНач",				Бюджетирование.ДатаНачалаПериода(ПрошлыйПериодПланирования, ПериодичностьПланирования));
	Запрос.УстановитьПараметр("ДатаКон",				Бюджетирование.ДатаКонцаПериода(ПрошлыйПериодПланирования, ПериодичностьПланирования));
	...
	Если РежимПланированияЗатрат = Перечисления.ВидыОрганизационнойСтруктурыПредприятия.ПоСтруктуреЮридическихЛиц Тогда
		...
		|	И НЕ (ОтражениеЗарплатыВРеглУчетеОтражениеВУчете.СчетДт В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСПерсоналомПоОплатеТруда), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоНалогам), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоСоциальномуСтрахованию)))
		|	И ОтражениеЗарплатыВРеглУчетеОтражениеВУчете.СчетКт В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСПерсоналомПоОплатеТруда), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСРазнымиДебиторамиИКредиторами), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоСоциальномуСтрахованию))
		|
		|СГРУППИРОВАТЬ ПО
...
Процедура ПроверитьЗаполнениеСтрокиПланируемыеЗатраты(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ВыборкаПоСтрокамДокумента.Подразделение.Пустая() И ВыборкаПоСтрокамДокумента.СтатьяЗатрат.Пустая() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "должно быть указано подразделение или статья затрат!", Отказ, Заголовок);
	КонецЕсли;
