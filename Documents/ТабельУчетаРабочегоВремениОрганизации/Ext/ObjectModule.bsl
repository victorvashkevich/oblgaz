...
Функция Автозаполнение(Запрос = Неопределено, Сотрудник = Неопределено, ТекущаяСтрокаТабличнойЧасти = Неопределено) Экспорт
	...
	Если Запрос = Неопределено Тогда 
		...
		Если Сотрудник = Неопределено Тогда
			...
			//vvv
			|	РаботникиОрганизаций.ВидУчетаВремени КАК ВидУчетаВремениСотрудника,
			//
			...
			//vvv
			|	РаботникиОрганизаций.ВидУчетаВремени,
			//
		...
		Иначе
			...
			//vvv
			|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций.СрезПоследних(&НачалоМесяца, Организация = &ГоловнаяОрганизация) КАК РаботникиОрганизаций
	...
	|					ВТСписокРаботников.Ссылка КАК Сотрудник
	|				ИЗ
	//vvv комментируем, добавляем условие на внутрисменность. Не будем брать внутрисменные состояния,т.к. они будут браться из расчетных документов
	...
	Иначе
		...
		| 		ТОГДА ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.ВыходныеДни)
		|		КОГДА ДниСотрудников.ВидВремени В (ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.Работа)) И (Сотрудники.ВидУчетаВремениСотрудника<>ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.ПустаяСсылка))
		|			ТОГДА Сотрудники.ВидУчетаВремениСотрудника
					...
					Пока Выборка.СледующийПоЗначениюПоля("День") Цикл
						...
						Пока Выборка.Следующий() Цикл 
							...
							Если ИндексМассиваПриставок > 3 Тогда
								...
								Прервать;
							КонецЕсли;
							//vvv явка с нулевым видом времени - это не явка
...
//vvv	
Процедура ВывестиИтогиПоСотруднику(ТекСотрудник,ТекДолжность,ТекСтавка,ТекПремия,ТаблицаИтогов,ТаблицаВыходных,ОбластьИтоги,Макет="Табель")
	
	ОбластьИтоги.Параметры.РабочихДней=0;		
	СтруктураПоиска.Вставить("Сотрудник",ТекСотрудник);
	СтруктураПоиска.Вставить("Должность",ТекДолжность);
	СтруктураПоиска.Вставить("Ставка",ТекСтавка);
	СтруктураПоиска.Вставить("ПроцентПремии",ТекПремия);
	...
	Если Макет="Табель1" ТОгда
				
		СтруктураПоиска.Вставить("Сотрудник",ТекСотрудник);
		СтруктураПоиска.Вставить("Должность",ТекДолжность);
		СтруктураПоиска.Вставить("Ставка",ТекСтавка);
		СтруктураПоиска.Вставить("ПроцентПремии",ТекПремия);
		СтруктураПоиска.Вставить("ВидИспользованияРабочегоВремени",Справочники.КлассификаторИспользованияРабочегоВремени.НайтиПоРеквизиту("БуквенныйКод","ДС"));
		
		НайденнаяСтрока=ТаблицаИтогов.НайтиСтроки(СтруктураПоиска);
		Если НайденнаяСтрока.Количество()<>0 Тогда
			ОбластьИтоги.Параметры.ДонорскихДней=ОбластьИтоги.Параметры.ДонорскихДней+НайденнаяСтрока[0].Дней;
		КонецЕсли;
		СтруктураПоиска.Очистить();
	...
	Если Макет="Табель" ТОгда
		ОбластьИтоги.Параметры.ПроцентПремии=0;
	КонецЕсли;
	...
	Если НайденнаяСтрока.Количество()<>0 Тогда
		...
		//ОбластьИтоги.Параметры.ПроцентПремии=НайденнаяСтрока[0].ПроцентПремии;
	КонецЕсли;
	Если Макет="Табель" ТОгда
		ОбластьИтоги.Параметры.ПроцентПремии=ТекПремия;
	КонецЕсли;
	СтруктураПоиска.Очистить();
...
Функция  ПечатьТабеля()
	...
	|	ПОМЕСТИТЬ ВТДатыСотрудниковБезДолжностей
	|ИЗ
	|	ВТДатыТабеля КАК ДатыТабеля,
	...
	ПоВсемТабелям = Не ЗначениеЗаполнено(ТабельУчетаОрганизации);
	ПоВсемПодразделениям = ?(ПоВсемТабелям,Не ЗначениеЗаполнено(ПодразделениеОрганизации),Истина);
	
	Запрос.УстановитьПараметр("ПоВсемТабелям",ПоВсемТабелям);
	Запрос.УстановитьПараметр("ПоВсемПодразделениям",ПоВсемПодразделениям);
	...
	Запрос.УстановитьПараметр("парамНачалоПериода",НачалоПериода);
	Запрос.УстановитьПараметр("парамКонецПериода",КонецПериода);
	...
	|	ДатыТабеля.ДатаТабеля,
	|	ВЫБОР
	|		КОГДА РаботникиОрганизаций.ПериодЗавершения <= ДатыТабеля.ДатаТабеля
	|				И РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	...
	|		КОГДА РаботникиОрганизаций.ПериодЗавершения <= ДатыТабеля.ДатаТабеля
	|				И РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА РаботникиОрганизаций.РазрядЕТСЗавершения
	|		ИНАЧЕ РаботникиОрганизаций.РазрядЕТС
	|	КОНЕЦ КАК РазрядЕТС,
	|	ВЫБОР
	|		КОГДА ТарифныеСтавкиРаботниковОрганизации.ПериодЗавершения <= ДатыТабеля.ДатаТабеля
	...
	|	КОНЕЦ КАК Ставка,
	|	ЕСТЬNULL(ДанныеОЕжемесячнойПремии.ПроцентПремии,0) КАК ПроцентПремии
	...
	|		МАКСИМУМ(ТарифныеСтавкиРаботниковОрганизации.Период) КАК ПериодСтавкиРаботника,
	|		МАКСИМУМ(ДанныеОЕжемесячнойПремии.Период) КАК ПериодПремии
	|	ИЗ
	|		ВТДатыСотрудниковБезДолжностей КАК ДатыТабеля
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|			ПО (РаботникиОрганизаций.Период <= ДатыТабеля.ДатаТабеля)
	|				И (РаботникиОрганизаций.Сотрудник = ДатыТабеля.Сотрудник)
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации КАК ТарифныеСтавкиРаботниковОрганизации
	|			ПО (ТарифныеСтавкиРаботниковОрганизации.Период <= ДатыТабеля.ДатаТабеля)
	|				И (ТарифныеСтавкиРаботниковОрганизации.Сотрудник = ДатыТабеля.Сотрудник)
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеОЕжемесячнойПремии КАК ДанныеОЕжемесячнойПремии
	|			ПО (ДанныеОЕжемесячнойПремии.Период <= ДатыТабеля.ДатаТабеля)
	|				И (ДанныеОЕжемесячнойПремии.Сотрудник = ДатыТабеля.Сотрудник)
	|				И (ДанныеОЕжемесячнойПремии.Регистратор=&Ссылка)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ДатыТабеля.ДатаТабеля,
	|		РаботникиОрганизаций.Сотрудник) КАК ДатыТабеля
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|		ПО (РаботникиОрганизаций.Период = ДатыТабеля.ПериодРаботника)
	|			И (РаботникиОрганизаций.Сотрудник = ДатыТабеля.Сотрудник)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации КАК ТарифныеСтавкиРаботниковОрганизации
	|		ПО (ТарифныеСтавкиРаботниковОрганизации.Период = ДатыТабеля.ПериодСтавкиРаботника)
	|			И (ТарифныеСтавкиРаботниковОрганизации.Сотрудник = ДатыТабеля.Сотрудник)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеОЕжемесячнойПремии КАК ДанныеОЕжемесячнойПремии
	|		ПО (ДанныеОЕжемесячнойПремии.Период = ДатыТабеля.ПериодПремии)
	|			И (ДанныеОЕжемесячнойПремии.Сотрудник = ДатыТабеля.Сотрудник)
	|				И (ДанныеОЕжемесячнойПремии.Регистратор=&Ссылка)
	|ГДЕ
	|	ВЫБОР
	...
	|	ДолжностиСотрудников.РазрядЕТС КАК РазрядЕТС,
	|	ДолжностиСотрудников.Ставка КАК Ставка,
	|	ЕСТЬNULL(ДолжностиСотрудников.ПроцентПремии,0) КАК ПроцентПремии
	|ПОМЕСТИТЬ ВТДатыСотрудников
	|ИЗ
	|	ВТДатыТабеля КАК ДатыТабеля
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДолжностиСотрудников КАК ДолжностиСотрудников
	|		ПО (ИСТИНА)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник,
	...
	|		ДатыСотрудников.Сотрудник.Наименование КАК СотрудникНаименование,
	|		ДатыСотрудников.Должность.Наименование КАК Должность,
	|		ДатыСотрудников.Должность.НаименованиеСокр КАК ДолжностьСокр,
	|		ДатыСотрудников.Должность КАК ДолжностьСсылка,
	|		ДатыСотрудников.РазрядЕТС КАК РазрядЕТС,
	|		ДатыСотрудников.Ставка КАК Оклад,
	|		ДатыСотрудников.ПроцентПремии КАК ПроцентПремии,
	|		ЗаполненныеДни.ВидВремени КАК ВидВремени,
	|		ЗаполненныеДни.Часов КАК Часов,
	|		ЗаполненныеДни.ДеньТабеля КАК ДеньТабеля
	|	ИЗ
	...
	|				ДатыСотрудников.Сотрудник КАК Сотрудник,
	|				ВЫБОР
	|					КОГДА РаботникиОрганизаций.ПериодЗавершения <= ДатыСотрудников.ДатаТабеля
	|							И РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|						ТОГДА РаботникиОрганизаций.ДолжностьЗавершения
	|					ИНАЧЕ РаботникиОрганизаций.Должность
	|				КОНЕЦ КАК Должность,
	...
	|				КОНЕЦ КАК РазрядЕТС,
	|				ВЫБОР
	|					КОГДА ТарифныеСтавкиРаботниковОрганизации.ПериодЗавершения <= ДатыСотрудников.ДатаТабеля
	|							И ТарифныеСтавкиРаботниковОрганизации.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|						ТОГДА ТарифныеСтавкиРаботниковОрганизации.СтавкаЗавершения
	|					ИНАЧЕ ТарифныеСтавкиРаботниковОрганизации.Ставка
	|				КОНЕЦ КАК Ставка,
	|				ЕСТЬNULL(ДанныеОЕжемесячнойПремии.ПроцентПремии,0) КАК ПроцентПремии,
	|				ДатыСотрудников.ВидВремени КАК ВидВремени,
	|				ДатыСотрудников.Часов КАК Часов,
	...
	|					ВЫБОР
	|						КОГДА ДанныеПоДням.ВидВремени = ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.ПустаяСсылка)
	|							ТОГДА """"
	|						КОГДА ДанныеПоДням.ВидВремени = ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.ВыходныеДни)
	|							ТОГДА ""В""
	...
	|						ИНАЧЕ ЕСТЬNULL(РабочееВремяРаботниковОрганизаций.ВидИспользованияРабочегоВремени.БуквенныйКод, """")
	...
	|							И ДатыСотрудников.Сотрудник = РабочееВремяРаботниковОрганизаций.Сотрудник
	...
	|							И (РаботникиОрганизаций.Период <= ДатыСотрудников.ДатаТабеля)
	|						ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации КАК ТарифныеСтавкиРаботниковОрганизации
	|						ПО (ТарифныеСтавкиРаботниковОрганизации.Сотрудник = ДатыСотрудников.Сотрудник)
	|							И (ТарифныеСтавкиРаботниковОрганизации.Период <= ДатыСотрудников.ДатаТабеля)
	|						ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеОЕжемесячнойПремии КАК ДанныеОЕжемесячнойПремии
	|						ПО (ДанныеОЕжемесячнойПремии.Сотрудник = ДатыСотрудников.Сотрудник)
	|							И (ДанныеОЕжемесячнойПремии.Период <= ДатыСотрудников.ДатаТабеля)
	|							И (ДанныеОЕжемесячнойПремии.Регистратор=&Ссылка)
	|						ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеПоДням КАК ДанныеПоДням
	|						ПО (ДанныеПоДням.Сотрудник = ДатыСотрудников.Сотрудник)
	|							И ДатыСотрудников.ДатаТабеля = ДанныеПоДням.ДатаТабеля
	|				
	|				СГРУППИРОВАТЬ ПО
	|					ДатыСотрудников.ДатаТабеля,
	...
	|							ТОГДА РабочееВремяРаботниковОрганизаций.Часов
	|						ИНАЧЕ ЕСТЬNULL(РабочееВремяРаботниковОрганизаций.ВидИспользованияРабочегоВремени.БуквенныйКод, """")
	|					КОНЕЦ) КАК ДатыСотрудников
	|					ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации КАК ТарифныеСтавкиРаботниковОрганизации
	|					ПО (ТарифныеСтавкиРаботниковОрганизации.Сотрудник = ДатыСотрудников.Сотрудник)
	|						И (ТарифныеСтавкиРаботниковОрганизации.Период = ДатыСотрудников.ПериодСтавкиРаботника)
	|					ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеОЕжемесячнойПремии КАК ДанныеОЕжемесячнойПремии
	|					ПО (ДанныеОЕжемесячнойПремии.Сотрудник = ДатыСотрудников.Сотрудник)
	|						И (ДанныеОЕжемесячнойПремии.Период = ДатыСотрудников.ПериодПремии)
	|						И (ДанныеОЕжемесячнойПремии.Регистратор=&Ссылка)
	|					ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|					ПО (РаботникиОрганизаций.Сотрудник = ДатыСотрудников.Сотрудник)
	|						И (РаботникиОрганизаций.Период = ДатыСотрудников.ПериодРаботника)) КАК ЗаполненныеДни
	|			ПО ДатыСотрудников.Сотрудник = ЗаполненныеДни.Сотрудник
	|				И ДатыСотрудников.ДатаТабеля = ЗаполненныеДни.ДатаТабеля
	|				И ДатыСотрудников.Ставка = ЗаполненныеДни.Ставка
	|				И ДатыСотрудников.ПроцентПремии = ЗаполненныеДни.ПроцентПремии
	|				И ДатыСотрудников.Должность = ЗаполненныеДни.Должность) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВложенныйЗапрос.СотрудникНаименование,
	|	ВложенныйЗапрос.Должность,
	...
	ЗапросИтоги.УстановитьПараметр("парамНачалоПериода",НачалоПериода);
	ЗапросИтоги.УстановитьПараметр("парамКонецПериода",КонецПериода);
	...
	// добавим, чтобы была возможность исключать внутрисменные отклонения из дней неявок (костыль)
	|	РабочееВремяРаботниковОрганизаций.Период КАК Период,
	|	ВЫБОР
	|		КОГДА РабочееВремяРаботниковОрганизаций.ВидИспользованияРабочегоВремени=ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.Работа) ТОГДА ""А""
	|		ИНАЧЕ ""Б""
	|	КОНЕЦ КАК ПорядокСортировки,
	//
	...
	|	КОЛИЧЕСТВО(*) КАК Дней,
	|	ВЫБОР
	|		КОГДА РаботникиОрганизаций.ПериодЗавершения <= РабочееВремяРаботниковОрганизаций.Период
	|				И РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА РаботникиОрганизаций.ДолжностьЗавершения
	|		ИНАЧЕ РаботникиОрганизаций.Должность
	|	КОНЕЦ КАК Должность,
	|	ВЫБОР
	|		КОГДА ТарифныеСтавкиРаботниковОрганизации.ПериодЗавершения <= РабочееВремяРаботниковОрганизаций.Период
	|				И ТарифныеСтавкиРаботниковОрганизации.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА ТарифныеСтавкиРаботниковОрганизации.СтавкаЗавершения
	|		ИНАЧЕ ТарифныеСтавкиРаботниковОрганизации.Ставка
	|	КОНЕЦ КАК Ставка,
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|			ПО (РаботникиОрганизаций.Сотрудник = РабочееВремяРаботниковОрганизаций.Сотрудник)
	|				И (РаботникиОрганизаций.Период <= РабочееВремяРаботниковОрганизаций.Период)
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации КАК ТарифныеСтавкиРаботниковОрганизации
	|			ПО (ТарифныеСтавкиРаботниковОрганизации.Сотрудник = РабочееВремяРаботниковОрганизаций.Сотрудник)
	|				И (ТарифныеСтавкиРаботниковОрганизации.Период <= РабочееВремяРаботниковОрганизаций.Период)
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеОЕжемесячнойПремии КАК ДанныеОЕжемесячнойПремии
	|			ПО (ДанныеОЕжемесячнойПремии.Сотрудник = РабочееВремяРаботниковОрганизаций.Сотрудник)
	|				И (ДанныеОЕжемесячнойПремии.Период <= РабочееВремяРаботниковОрганизаций.Период)
	|				И (ДанныеОЕжемесячнойПремии.Регистратор=&Регистратор)
	|	ГДЕ
	...
	|		РабочееВремяРаботниковОрганизаций.Часов,
	|		РабочееВремяРаботниковОрганизаций.Дней) КАК РабочееВремяРаботниковОрганизаций
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|		ПО (РаботникиОрганизаций.Сотрудник = РабочееВремяРаботниковОрганизаций.Сотрудник)
	|			И (РаботникиОрганизаций.Период = РабочееВремяРаботниковОрганизаций.ПериодРаботника)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации КАК ТарифныеСтавкиРаботниковОрганизации
	|		ПО (ТарифныеСтавкиРаботниковОрганизации.Сотрудник = РабочееВремяРаботниковОрганизаций.Сотрудник)
	|			И (ТарифныеСтавкиРаботниковОрганизации.Период = РабочееВремяРаботниковОрганизаций.ПериодСтавкиРаботника)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеОЕжемесячнойПремии КАК ДанныеОЕжемесячнойПремии
	|		ПО (ДанныеОЕжемесячнойПремии.Сотрудник = РабочееВремяРаботниковОрганизаций.Сотрудник)
	|			И (ДанныеОЕжемесячнойПремии.Период = РабочееВремяРаботниковОрганизаций.ПериодПремии)
	|			И (ДанныеОЕжемесячнойПремии.Регистратор=&Регистратор)
	|
	|СГРУППИРОВАТЬ ПО
	|	РабочееВремяРаботниковОрганизаций.Сотрудник,
	...
	|		ИНАЧЕ РаботникиОрганизаций.Должность
	|	КОНЕЦ,
	|	ЕСТЬNULL(ДанныеОЕжемесячнойПремии.ПроцентПремии,0),
	|	ВЫБОР
	|		КОГДА ТарифныеСтавкиРаботниковОрганизации.ПериодЗавершения <= РабочееВремяРаботниковОрганизаций.Период
	|				И ТарифныеСтавкиРаботниковОрганизации.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА ТарифныеСтавкиРаботниковОрганизации.СтавкаЗавершения
	...
	Если ИмяМакета="Табель1" ТОгда
		...
		ТаблицаИтогов.Сортировать("Сотрудник,Период,ПорядокСортировки");
		ТекДата="";
		ТекСотрудник="";
		Для Каждого Стр Из ТаблицаИтогов Цикл
			Если (ТекДата=Стр.Период) И (ТекСотрудник=Стр.Сотрудник) Тогда
	...
	ТаблицаИтогов.Свернуть("Сотрудник,ВидИспользованияРабочегоВремени,Должность,Ставка,ПроцентПремии","Дней,Часов");
	...
	ЗапросПоВыходным.МенеджерВременныхТаблиц=Новый МенеджерВременныхТаблиц;
	ЗапросПоВыходным.Текст=
	"ВЫБРАТЬ
	| ВыходныеДни.Сотрудник,
	| ВыходныеДни.ПроцентПремии,
	| ВыходныеДни.ДатаТабеля	
	...
	Пока ДатаВОтчете <= НачалоДня(КонецПериода) Цикл
		...
		|	1 КАК Количество,
		|	ДАТАВРЕМЯ(" + Формат(Год(ДатаВОтчете), "ЧГ=4") + ", " + Месяц(ДатаВОтчете) + " , "+НомерДня+") КАК ДатаТабеля,
		|	ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.ПервыйВидВремени"+НомерДня+""+
		" ИЗ
		|	Документ.ТабельУчетаРабочегоВремениОрганизации.ОтработанноеВремя КАК ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя
		|ГДЕ
		|	ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.Ссылка = &ДокументСсылка
		|	И ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.ПервыйВидВремени"+НомерДня+"=ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.ВыходныеДни)"+Символы.ПС;
		
		Если ДатаВОтчете<НачалоДня(КонецПериода) ТОгда
			ЗапросПоВыходным.Текст=ЗапросПоВыходным.Текст+
			"	ОБЪЕДИНИТЬ ВСЕ
			|";
		КОнецЕсли;
	...
	|	ВыходныеДни.Должность,
	|	ВыходныеДни.Ставка
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВыходныеДни.Сотрудник КАК Сотрудник,
	|		ВыходныеДни.ДатаТабеля КАК ДатаТабеля,
	|		ВыходныеДни.ПроцентПремии КАК ПроцентПремии,
	|		1 КАК Количество,
	|		ВЫБОР
	|			КОГДА РаботникиОрганизаций.ПериодЗавершения <= ВыходныеДни.ДатаТабеля
	|					И РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА РаботникиОрганизаций.ДолжностьЗавершения
	|			ИНАЧЕ РаботникиОрганизаций.Должность
	|		КОНЕЦ КАК Должность,
	|		ВЫБОР
	|			КОГДА ТарифныеСтавкиРаботниковОрганизации.ПериодЗавершения <= ВыходныеДни.ДатаТабеля
	|					И ТарифныеСтавкиРаботниковОрганизации.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ТарифныеСтавкиРаботниковОрганизации.СтавкаЗавершения
	|			ИНАЧЕ ТарифныеСтавкиРаботниковОрганизации.Ставка
	|		КОНЕЦ КАК Ставка
	...
	|			МАКСИМУМ(ТарифныеСтавкиРаботниковОрганизации.Период) КАК ПериодСтавкиРаботника
	|		ИЗ
	|			ВТВыходныеДни КАК ВыходныеДни
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|				ПО (РаботникиОрганизаций.Период <= ВыходныеДни.ДатаТабеля)
	|					И (РаботникиОрганизаций.Сотрудник = ВыходныеДни.Сотрудник)
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации КАК ТарифныеСтавкиРаботниковОрганизации
	|				ПО (ТарифныеСтавкиРаботниковОрганизации.Период <= ВыходныеДни.ДатаТабеля)
	|					И (ТарифныеСтавкиРаботниковОрганизации.Сотрудник = ВыходныеДни.Сотрудник)
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ВыходныеДни.Сотрудник,
	|			ВыходныеДни.ПроцентПремии,
	|			ВыходныеДни.ДатаТабеля) КАК ВыходныеДни
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|			ПО (РаботникиОрганизаций.Период = ВыходныеДни.ПериодРаботника)
	|				И (РаботникиОрганизаций.Сотрудник = ВыходныеДни.Сотрудник)
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации КАК ТарифныеСтавкиРаботниковОрганизации
	|			ПО (ТарифныеСтавкиРаботниковОрганизации.Период = ВыходныеДни.ПериодСтавкиРаботника)
	|				И (ТарифныеСтавкиРаботниковОрганизации.Сотрудник = ВыходныеДни.Сотрудник)) КАК ВыходныеДни
	|
	|СГРУППИРОВАТЬ ПО
	|	ВыходныеДни.Сотрудник,
	|	ВыходныеДни.ПроцентПремии,
	...
	МассивИтогов[0]=Справочники.КлассификаторИспользованияРабочегоВремени.Работа;
	МассивИтогов[1]=Справочники.КлассификаторИспользованияРабочегоВремени.Болезнь;
	МассивИтогов[2]=Справочники.КлассификаторИспользованияРабочегоВремени.ОсновнойОтпуск;
	МассивИтогов[3]=Справочники.КлассификаторИспользованияРабочегоВремени.РаботаНочныеЧасы;
	МассивИтогов[4]=Справочники.КлассификаторИспользованияРабочегоВремени.РаботаВечерниеЧасы;
	МассивИтогов[5]=Справочники.КлассификаторИспользованияРабочегоВремени.Праздники;
	...
	ТекПремия="";
	НомерПП=1;
	Пока ВыборкаПоСотрудникам.СледующийПоЗначениюПоля("Должность") Цикл
		Пока ВыборкаПоСотрудникам.СледующийПоЗначениюПоля("Оклад") Цикл
			Пока ВыборкаПоСотрудникам.СледующийПоЗначениюПоля("ПроцентПремии") Цикл
				Пока ВыборкаПоСотрудникам.СледующийПоЗначениюПоля("Сотрудник") Цикл
			
					Если ТекСотрудник<>"" Тогда
						ВывестиИтогиПоСотруднику(ТекСотрудник,ТекДолжность,ТекСтавка,ТекПремия,ТаблицаИтогов,ТаблицаВыходных,ОбластьИтоги,ИмяМакета);
					...
					Если ИмяМакета="Табель1" ТОгда
						ОбластьСотрудник.Параметры.НомерПП=НомерПП;
						НомерПП=НомерПП+1;
					КонецЕсли;					
					...
					Пока ВыборкаПоСотрудникам.СледующийПоЗначениюПоля("ДатаТабеля") Цикл
						...
						Пока ВыборкаПоСотрудникам.СледующийПоЗначениюПоля("ДеньТабеля") Цикл
							ОбластьТабеля=?(ВыборкаПоСотрудникам.ДеньТабеля="В",ОбластьВыходнойДень,ОбластьДень);
							ДеньТабеля=ДеньТабеля+Строка(ВыборкаПоСотрудникам.ДеньТабеля)+?(ДеньТабеля<>"",ВыборкаПоСотрудникам.Часов,"")+Символы.ПС;	
						КонецЦикла;
					...
					ТекДолжность=ВыборкаПоСотрудникам.ДолжностьСсылка;
					ТекСтавка=ВыборкаПоСотрудникам.Оклад;
					ТекПремия=ВыборкаПоСотрудникам.ПроцентПремии;
	...
	ОбластьИтогоДнейКонец.Параметры.ИтогоДонорскихДней=0;
	Для Каждого Стр Из ТаблицаИтогов Цикл
		
		Если Стр.ВидИспользованияРабочегоВремени=Справочники.КлассификаторИспользованияРабочегоВремени.Работа ТОгда	  	
			ОбластьИтогоДнейКонец.Параметры.ИтогоРабочихДней=Стр.Дней;		
			ОбластьИтогоДнейКонец.Параметры.ИтогоРабочихЧасов=Стр.Часов;
		...
		ИначеЕсли Стр.ВидИспользованияРабочегоВремени=Справочники.КлассификаторИспользованияРабочегоВремени.НайтиПоРеквизиту("БуквенныйКод","Д") ТОгда		
			ОбластьИтогоДнейКонец.Параметры.ИтогоДонорскихДней=ОбластьИтогоДнейКонец.Параметры.ИтогоДонорскихДней+Стр.Дней;
		ИначеЕсли Стр.ВидИспользованияРабочегоВремени=Справочники.КлассификаторИспользованияРабочегоВремени.НайтиПоРеквизиту("БуквенныйКод","ДС") ТОгда		
			ОбластьИтогоДнейКонец.Параметры.ИтогоДонорскихДней=ОбластьИтогоДнейКонец.Параметры.ИтогоДонорскихДней+Стр.Дней;	
...
Функция СформироватьЗапросПоОтработанноеВремя(ВыборкаПоШапкеДокумента, ТаблицаСотрудниковСПустымиЯчейками)
	...
	//vvv закоментируем, реализуем возможность прямо в табеле в соответствующей ячейке указывать, что день оплачивается как праздничный ( в двойном размере) Я+РВ
	//|	ВЫБОР
	...
	//|	КОНЕЦ КАК ВидВремени,
	|	ДанныеТабеляПоДнямДляУчетаПолнотыЗаполнения.ВидВремени КАК ВидВремени,
	//
	...
	|	(НЕ ДанныеТабеляПоДнямДляУчетаПолнотыЗаполнения.ВидВремени В (ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.ПустаяСсылка)))
	//
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник
...
//	Нет.
//
Процедура ДобавитьСтрокуРабочееВремяРаботниковОрганизацийПоДням(ВыборкаПоСтрокамДокумента, ВыборкаПоШапкеДокумента, НаборЗаписей, ДопЯвка=Ложь)
	Движение = НаборЗаписей.Добавить();
	...
	Если ДопЯвка Тогда
		...
		// Ресурсы
		Движение.Дней								= ?(ВремяДопЯвки = 0, 0, 1);
		Движение.Часов								= ВремяДопЯвки;
	Иначе
    //
...
Процедура ДобавитьСтрокуГрафикиРаботыПоВидамВремениЧасы(ВыборкаПоСтрокамДокумента, ВыборкаПоШапкеДокумента, ТаблицаНабора, ЗаполнятьРеквизитДокумент = Истина)
	...
	ИначеЕсли (Движение.Документ = Документы.ТабельУчетаРабочегоВремениОрганизации.ПустаяСсылка()) И НЕ ВыборкаПоСтрокамДокумента.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.Праздники Тогда	
	...
	//vvv комментируем, добавляем условие, если праздничные (РВ) не увеличиваем отраб. время, т.к. это просто признак оплаты в двойном размере	
...
Функция СформироватьЗапросПоПремии(ВыборкаПоШапкеДокумента)
	...
	Запрос.Текст=Запрос.Текст+
		" УПОРЯДОЧИТЬ ПО Сотрудник, ИндексДня
		|";
								...
								Если (ВыборкаПоОтработанноеВремя.ДеньМесяца <= ДеньОкончанияПериода)
									...
									Пока ВыборкаПоОтработанноеВремя.Следующий() Цикл
										...
										Если НЕ ВыборкаПоОтработанноеВремя.ВидВремени.Пустая() Тогда
											...
											КонецЕсли;
											//vvv
											Если ВыборкаПоОтработанноеВремя.ВидВремени.ДополнительноУчитыватьКакЯвкуВТабеле ТОгда
												ВремяДопЯвки=ВремяДопЯвки+ВыборкаПоОтработанноеВремя.Часов;
											КонецЕсли;
											//
									...
									КонецЦикла;
									//vvv
									Если ВремяДопЯвки<>0 Тогда
										ДобавитьСтрокуРабочееВремяРаботниковОрганизацийПоДням(ВыборкаПоОтработанноеВремя, ВыборкаПоШапкеДокумента, Движения.РабочееВремяРаботниковОрганизаций,Истина);	
									КонецЕсли;
									//
	...
	//			КонецЦикла;
				...
				Пока ВыборкаПоПремии.Следующий() Цикл
					Если ВыборкаПоПремии.ПроцентПремии>0 Тогда
						...
						Если ЗначениеЗаполнено(ВыборкаПоПремии.ВидВремени) Тогда
							...
							Движение.Месяц								= ВыборкаПоШапкеДокумента.ПериодРегистрации;						
	                    	Движение.Период								= ВыборкаПоПремии.ДатаТабеля;
