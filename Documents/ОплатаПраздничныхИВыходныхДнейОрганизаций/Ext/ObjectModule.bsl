...
Функция ЗаполнитьРаботавшимиВПраздники() Экспорт
	...
	|			ТОГДА ГрафикиРаботыПоВидамВремени.ДополнительноеЗначениеПолное
	...
	|ИЗ
	|	ВТСводныеИндивидуальныеГрафикиРаботы КАК СписокРаботников
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СводныеИндивидуальныеГрафики КАК СводныеИндивидуальныеГрафики
	|		ПО СписокРаботников.Сотрудник = СводныеИндивидуальныеГрафики.Сотрудник
	|			И СписокРаботников.ТекущийМесяц = СводныеИндивидуальныеГрафики.Месяц
	|ГДЕ
	...
	|	СписокРаботников.Сотрудник КАК Сотрудник,
	|	СписокРаботников.ТекущийМесяц
	|ПОМЕСТИТЬ ВТСписокРаботниковПоЕжедневнымИндивидуальнымГрафикам
	|ИЗ
	|	ВТСотрудникиПоПраздникам КАК СписокРаботников
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ГрафикиРаботыПоВидамВремени
	|		ПО СписокРаботников.Сотрудник = ГрафикиРаботыПоВидамВремени.ГрафикРаботы
	...
	|ИЗ
	|	ВТСписокРаботниковПоЕжедневнымИндивидуальнымГрафикам КАК СписокРаботников
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ГрафикиРаботыПоВидамВремени
	|		ПО СписокРаботников.Сотрудник = ГрафикиРаботыПоВидамВремени.ГрафикРаботы
	|			И СписокРаботников.ТекущийМесяц = ГрафикиРаботыПоВидамВремени.Месяц
	|			И (ГрафикиРаботыПоВидамВремени.План)
	...
	//vvv
	|	СотрудникиПоПраздникам.Должность,
	//
	...
	|	ВЫБОР
	|		КОГДА ПлановыеНачисленияРаботниковОрганизаций.СпособРасчета В (ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.ПоЧасовойТарифнойСтавке), ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.СдельныйЗаработок), ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.СделкаПоЧасовойТарифнойСтавке))
	...
	|										ТОГДА 0                                           
	|									ИНАЧЕ ПлановыеНачисленияРаботниковОрганизаций.Размер /ЕСТЬNULL(РазмерыПараметровДляРасчетаРБ.Размер,1)
	|								КОНЕЦ
	|						КОГДА ВЫБОР
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Результат,
	|	ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаПраздничныеИВыходныеПоГрафику) КАК ВидРасчета
	|ИЗ
	...
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РазмерыПараметровДляРасчетаРБ.СрезПоследних(&ПериодРегистрации,ВидПараметра = &СреднеМесячноеКоличесвтоЧасов)  КАК РазмерыПараметровДляРасчетаРБ
	|		ПО (ИСТИНА)
	|ГДЕ
	|	ВЫБОР
	|			КОГДА ФактическиОтработано.Сотрудник ЕСТЬ НЕ NULL 
	...
	тт = ОбщегоНазначения.ПолучитьВТ(Запрос.МенеджерВременныхТаблиц, "ВТОбщиеГрафикиРаботы");
...
Функция ЗаполнитьРаботавшимиПоНарядам() Экспорт
	...
	|	И ФактическаяВыработкаРаботниковОрганизацийСверхурочно.ВидНаряда = ЗНАЧЕНИЕ(Перечисление.ВидыНарядов.ВыходнойНаряд)
	...
	Если Выборка.Количество() > 0 тогда
		...
		Пока Выборка.Следующий() цикл
			...
			Строка.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаЗаРаботуВПраздникиИВыходныеПоНарядам;
			Строка.ОтработаноЧасов = Выборка.ОтработаноЧасов;
			...
			Иначе
				...
				//
				Строка.ДатаВыхода = Выборка.Период;
				Строка.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаРаботуВПраздникиИВыходныеПоНарядам;
				Строка.ОтработаноЧасов = Выборка.ОтработаноЧасов;
				Строка.Размер = ПроцедурыУправленияПерсоналом.ЧасоваяТарифнаяСтавкаРаботникаОрг(Выборка.Сотрудник, Выборка.Период, НачалоМесяца(ПериодРегистрации), мВалютаРегламентированногоУчета);
...
Процедура Перерассчитать() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		//vvv
		СтрокаНачисления.Должность = Выборка.НоваяДолжность;
		//
...
Процедура Автозаполнение(Запрос = Неопределено, Реквизиты = Неопределено) Экспорт
	...
	|	СписокРаботников.Сотрудник КАК Сотрудник,
	|	ГрафикиРаботыПоВидамВремени.План,
	|	ГрафикиРаботыПоВидамВремени.ОсновноеЗначениеНорма КАК Часы
	|ПОМЕСТИТЬ ВТДанныеИндивидуальныхГрафиковИТабелей
	|ИЗ
	|	ВТСписокРаботников КАК СписокРаботников
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ГрафикиРаботыПоВидамВремени
	...
	//vvv
	|	РаботникиИНачисления.Должность,
	//
	...
	//vvv
	|		СписокРаботников.Должность КАК Должность,
	//
	...
	|	ВЫРАЗИТЬ(СписокРаботников.ОтработаноЧасов * СписокРаботников.Размер КАК ЧИСЛО(15, 0))  КАК Результат,
	|	ВЫБОР
	...
	|	ВЫРАЗИТЬ(СписокРаботников.ОтработаноЧасов * СписокРаботников.Размер КАК ЧИСЛО(15, 0)),
...
Процедура ЗаполнитьПоПерерассчитываемомуДокументу(ИсходныйДокумент, Сотрудники = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		//vvv
		НоваяСтрока.Должность					= Выборка.НоваяДолжность;
		//
		НоваяСтрока.ДатаВыхода					= Выборка.ДатаВыхода;
		НоваяСтрока.ВидРасчета					= Выборка.ВидРасчета;
...
Функция СформироватьЗапросДляПерерасчета(ДокументСсылка, ПериодРегистрацииДокумента, Сотрудники = Неопределено, ПоЗаписямПерерасчета = Ложь)
	...
	//vvv
	|	Начисления.Должность,
	|	РаботникиИНачисления.НоваяДолжность,
	//
	|	Начисления.ОтработаноЧасов,
