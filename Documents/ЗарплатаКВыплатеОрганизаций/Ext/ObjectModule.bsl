...
Функция РассчитатьТаблицуЗарплат(ТаблицаЗарплат) Экспорт
	...
	Для Каждого СтрокаЗарплаты Из ТаблицаЗарплат Цикл
		...
		Если ПредварительныйУчетУдержаний Тогда
			...
			//vvv
			УдерПлан = ?(СтрокаПлан = Неопределено, 0,Окр(СтрокаЗарплаты.Сумма * СтрокаПлан.Процент/100, ?(Дата>=ОбщегоНазначения.ПолучитьДатуДеноминации(),2,0)));
			//
			...
			//vvv
			УдерНДФЛ = ?(СтрокаНДФЛ = Неопределено, 0, Окр(СтрокаЗарплаты.Сумма * СтрокаНДФЛ.Ставка/100, ?(Дата>=ОбщегоНазначения.ПолучитьДатуДеноминации(),2,0)));
			//
		...
		Если ЗначениеЗаполнено(Округление) Тогда
			...
			Если	Округление = Перечисления.СпособыОкругленияЗарплатыКВыплате.ПоПостановлению Тогда  
				ТекущаяСтрока = СтрокаЗарплаты.Сумма;
				Остаток = ТекущаяСтрока%100;
				Если Остаток<=49 Тогда
					СтрокаЗарплаты.Сумма = Окр(СтрокаЗарплаты.Сумма,-2,0);
				ИначеЕсли Остаток>50 Тогда
					СтрокаЗарплаты.Сумма = Окр(СтрокаЗарплаты.Сумма,-2,1);
				КонецЕсли;
           	Иначе	
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат ЗарплатаКВыплатеОрганизацийПереопределяемый.ПолучитьСтруктуруПечатныхФорм()
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если НЕ Отказ И УпрощенныйУчетВзаиморасчетов Тогда
		...
		ЗарплатаКВыплатеОрганизацийПереопределяемый.ПровестиВзаиморасчеты(ЭтотОбъект);
