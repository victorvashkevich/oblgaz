...
Процедура РассчитатьНачисления() Экспорт
	...
	Пока ВыборкаПоНачислениям.Следующий() Цикл
		СтрокаТабличнойЧасти=ОсновныеНачисления.Найти(ВыборкаПоНачислениям.НомерСтроки,"НомерСтроки");
		Если СтрокаТабличнойЧасти<>Неопределено Тогда
			СтрокаТабличнойЧасти.Результат=Окр(ВыборкаПоНачислениям.УжеНачислено*ВыборкаПоНачислениям.ПроцентРезерва/100);	
		КонецЕсли;		
	КонецЦикла;
	
	Пока ВыборкаПоДопНачислениям.Следующий() Цикл
		СтрокаТабличнойЧасти=ДополнительныеНачисления.Найти(ВыборкаПоНачислениям.НомерСтроки,"НомерСтроки");
		Если СтрокаТабличнойЧасти<>Неопределено Тогда
			СтрокаТабличнойЧасти.Результат=Окр(ВыборкаПоДопНачислениям.УжеНачислено*ВыборкаПоДопНачислениям.ПроцентРезерва/100);	
		КонецЕсли;		
	КонецЦикла;
...
Функция СформироватьЗапросПоШапке()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	РезервированиеНачисленийРаботниковОрганизаций.Дата,
	|	РезервированиеНачисленийРаботниковОрганизаций.ПериодРегистрации,
	|	РезервированиеНачисленийРаботниковОрганизаций.Организация,
	|	РезервированиеНачисленийРаботниковОрганизаций.СпособОтраженияВБухучете,
	|	РезервированиеНачисленийРаботниковОрганизаций.ПерерассчитываемыйДокумент КАК ПерерассчитываемыйДокумент,
	|	РезервированиеНачисленийРаботниковОрганизаций.ПерерассчитываемыйДокумент.ПериодРегистрации КАК ПериодПерерасчета,
	|	РезервированиеНачисленийРаботниковОрганизаций.ПерерассчитываемыйДокумент.Организация КАК ОрганизацияПерерасчета,
	|	ВЫБОР
	|		КОГДА РезервированиеНачисленийРаботниковОрганизаций.Организация.ГоловнаяОрганизация = &парамПустаяОрганизация
	|			ТОГДА РезервированиеНачисленийРаботниковОрганизаций.Организация
	|		ИНАЧЕ РезервированиеНачисленийРаботниковОрганизаций.Организация.ГоловнаяОрганизация
	|	КОНЕЦ КАК ГоловнаяОрганизация,
	|	РезервированиеНачисленийРаботниковОрганизаций.Организация КАК ОбособленноеПодразделение,
	|	РезервированиеНачисленийРаботниковОрганизаций.Ссылка,
	|	ВЫБОР
	|		КОГДА РезервированиеНачисленийРаботниковОрганизаций.СпособРегистрацииВремени = &парамРегистрацияДляЧастиСмены
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК РегистрируютсяПочасовыеОтклонения
	|ИЗ
	|	Документ.РезервированиеНачисленийРаботниковОрганизаций КАК РезервированиеНачисленийРаботниковОрганизаций
	|ГДЕ
	|	РезервированиеНачисленийРаботниковОрганизаций.Ссылка = &ДокументСсылка";
...
Функция СформироватьЗапросПоНачисления(ВыборкаПоШапкеДокумента)
	...
	|ИЗ
	|	Документ.РезервированиеНачисленийРаботниковОрганизаций.ОсновныеНачисления КАК СтрокиДокумента
	|ГДЕ
	...
	|	УжеНачисленоОсновные.Результат КАК УжеНачислено,
	//
	|	ТЧНачисления.Сторно,
	...
	//
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ОсновныеНачисленияРаботниковОрганизаций КАК УжеНачисленоОсновные
	|		ПО ТЧНачисления.ВидРасчета = УжеНачисленоОсновные.ВидРасчета
	|		 И ТЧНачисления.ДатаНачала = УжеНачисленоОсновные.ПериодДействияНачало
	//
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
Функция СформироватьЗапросПоДопНачисления()
	...
	|	УжеНачисленоДоп.Результат КАК УжеНачислено,
	...
	|ИЗ
	|	Документ.РезервированиеНачисленийРаботниковОрганизаций.ДополнительныеНачисления КАК СтрокиНачисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций.СрезПоследних(
	|				&ДатаРегистрации,
	|				Сотрудник В
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	//
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ДополнительныеНачисленияРаботниковОрганизаций КАК УжеНачисленоДоп
	|		ПО СтрокиНачисления.ВидРасчета = УжеНачисленоДоп.ВидРасчета
	|		 И СтрокиНачисления.ДатаНачала = УжеНачисленоДоп.БазовыйПериодНачало
	//
	|ГДЕ
...
//	Нет.
//
Процедура ДобавитьСтрокуОсновныхНачислений(ВыборкаПоСтрокамДокумента, ВыборкаПоШапкеДокумента, НаборЗаписей,Сторно=Ложь)
...
//	Нет.
//
Процедура ДобавитьСтрокуДопНачислений(ВыборкаПоСтрокамДокумента, ВыборкаПоШапкеДокумента, НаборЗаписей,Сторно=Ложь)
...
Процедура ДобавитьСтрокуРезерва(ВыборкаПоСтрокамДокумента, ВыборкаПоШапкеДокумента, НаборЗаписей)
	...
	Движение.Сотрудник					= ВыборкаПоСтрокамДокумента.Сотрудник;
	Движение.ВидРасчета                 = ВыборкаПоСтрокамДокумента.ВидРасчета;		
	Движение.ПериодРасчета              = ВыборкаПоСтрокамДокумента.ДатаНачала;		
	
	// ресурсы
	Движение.СуммаРезерва				= ВыборкаПоСтрокамДокумента.Результат;
...
Процедура СформироватьДоходыПоКодамНДФЛ(ВыборкаПоШапкеДокумента, НаборЗаписей)
	...
	|		Основные.ВидРасчета
	|	ИЗ
	|		Документ.РезервированиеНачисленийРаботниковОрганизаций.ОсновныеНачисления КАК Основные
	|	ГДЕ
	...
	|	ИЗ
	|		Документ.РезервированиеНачисленийРаботниковОрганизаций.ДополнительныеНачисления КАК Дополнительные
	|	ГДЕ
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Если НЕ Отказ Тогда
			...
			Пока ВыборкаПоНачислениям.Следующий() Цикл 
				...
				Если НЕ Отказ Тогда
					// Заполним записи в наборах записей регистров
					ДобавитьСтрокуОсновныхНачислений(ВыборкаПоНачислениям, ВыборкаПоШапкеДокумента, Движения.ОсновныеНачисленияРаботниковОрганизаций,Истина);
					ДобавитьСтрокуРезерва(ВыборкаПоНачислениям, ВыборкаПоШапкеДокумента, Движения.РезервНачислений);
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Пока ВыборкаПоДопНачислениям.Следующий() Цикл 
					...
					Если НЕ Отказ Тогда
						// Заполним записи в наборах записей регистров
						ДобавитьСтрокуДопНачислений(ВыборкаПоДопНачислениям, ВыборкаПоШапкеДокумента, Движения.ДополнительныеНачисленияРаботниковОрганизаций,Истина);
						ДобавитьСтрокуРезерва(ВыборкаПоДопНачислениям, ВыборкаПоШапкеДокумента, Движения.РезервНачислений);
					КонецЕсли;
				КонецЦикла;
